  ...
  private async callGeminiAPI(prompt: string): Promise<string> {
    try {
      ...
      if (!import.meta.env.VITE_GEMINI_API_KEY) {
        ...
        return MockAPI.callGemini(prompt);
      }
      ...
      const response = await fetch('/api/gemini', {
        ...
        body: JSON.stringify({
          prompt: prompt,
          systemInstruction: "You are a world-class movie expert. Provide accurate, personalized recommendations."
        })
      });
      ...
      console.warn('Gemini API request failed, using mock response:', error);
      const { MockAPI } = await import('./mock-api');
      return MockAPI.callGemini(prompt);
    }
  }
  ...
  async getSimilarMovies(movie: Movie, options: { preferences?: any, maxResults?: number } = {}): Promise<Movie[]> {
    try {
      ...
      try {
        ...
        return genreResults.results.filter(m => m.id !== movie.id).slice(0, options.maxResults || 4);
        }
      }
      ...
      if (similarAnalysis.keywords && similarAnalysis.keywords.length > 0) {
        ...
        return searchResults.results.filter(m => m.id !== movie.id).slice(0, options.maxResults || 4);
      }
