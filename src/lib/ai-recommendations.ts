...
// Enhanced AI recommendation service using Gemini AI
export const aiRecommendationEngine = {
  ...
  async getSimilarMovies(movie: Movie): Promise<Movie[]> {
    try {
      ...
      console.error('Gemini AI failed for similar movies, falling back:', error);
  ...
  async getMoodBasedRecommendations(mood: string): Promise<Movie[]> {
    try {
      ...
      console.error('Gemini AI failed for mood recommendations, falling back:', error);
