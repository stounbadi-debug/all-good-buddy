...
const EnhancedProfileBuilder: React.FC = () => {
  ...
  const validateProfile = useCallback((profile: any): PersonProfile => {
    if (!profile || typeof profile !== 'object') {
      console.warn('Invalid profile object, creating default profile');
      return {
        name: 'Unknown',
        jobTitle: '',
        company: '',
        ...
        dataQuality: { completeness: 0.5, accuracy: 0.5, consistency: 0.5 },
        errors: ['Profile validation failed - using default values'],
        warnings: []
      };
    }
    ...
    return {
      ...
      sourceUrl: profile.sourceUrl || state.url,
      generatedAt: profile.generatedAt || new Date().toISOString(),
      dataQuality: profile.dataQuality || { completeness: 0.5, accuracy: 0.5, consistency: 0.5 },
  ...
  const handleGenerateProfile = async () => {
    ...
    try {
      ...
      // Validate and ensure profile integrity
      ...
      const confidenceValue = profile?.confidence ?? 0.5;
      toast({
        title: 'Profile Generated Successfully!',
        description: `Extracted with ${Math.round(confidenceValue * 100)}% confidence in ${(processingTime / 1000).toFixed(1)}s`,
      });
